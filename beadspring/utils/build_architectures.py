...
def create_polymer_matrix(n, m, l=0.97, R=0.5, file_name=None):
    ...
    for i in range(len(pos)):
        for j in np.arange(i, len(pos), 1):
            if (
                (pos[i, 0] == pos[j, 0] - 1 or pos[i, 0] == pos[j, 0] - xmax + 1)
                and pos[i, 1] == pos[j, 1]
                and pos[i, 2] == pos[j, 2]
            ):
                bonda += [i + 1]
                ...
                pos[i, 0] == pos[j, 0]
                and (pos[i, 1] == pos[j, 1] - 1 or pos[i, 1] == pos[j, 1] - xmax + 1)
                and pos[i, 2] == pos[j, 2]
            ):
                bonda += [i + 1]
                bondb += [j + 1]
                ...
                pos[i, 0] == pos[j, 0]
                and pos[i, 1] == pos[j, 1]
                and (pos[i, 2] == pos[j, 2] - 1 or pos[i, 2] == pos[j, 2] - xmax + 1)
            ):
                bonda += [i + 1]
                bondb += [j + 1]
...
def create_star(num_arms, arm_length, core_radius=0.5, file_name=None):
    ...
    for j in range(num_arms):  # chain loop
        ...
        for i in range(arm_length):  # monomer loop for each chain
            if i == 0:
                ...
                xij = (
                    (R + 0.5) * np.sin(ph) * np.cos(th)
                )  # initial position for each chain
                yij = (R + 0.5) * np.sin(ph) * np.sin(th)
                zij = (R + 0.5) * np.cos(ph)
            else:
                restriction = (
                    True  # coordinate of the first B atom for the first A atom
                )
                while restriction:
                    ...
                    if i >= 2:
                        ...
                        if (
                            np.sqrt(distx * distx + disty * disty + distz * distz)
                            <= dmin
                        ):  # the minimum distance between atom 1 and atom 3
                            restriction = True
                        elif (
                            (xij < xlo)
                            or (xij > xhi)
    ...
    for k in range(num_arms):
        ...
        bond[(k * (arm_length - 1)) : (k + 1) * (arm_length - 1), 2] = np.arange(
            k * arm_length + 2, (k + 1) * arm_length + 1
        )
        bond[(k * (arm_length - 1)) : (k + 1) * (arm_length - 1), 3] = np.arange(
            k * arm_length + 3, (k + 1) * arm_length + 2
        )
...
def create_polydisperse_star(f1, M1, f2, M2, file_name=None):
    ...
    for k in range(f1):
        ...
        bond[(k * (M1 - 1)) : (k + 1) * (M1 - 1), 2] = np.arange(
            k * M1 + 2, (k + 1) * M1 + 1
        )
        bond[(k * (M1 - 1)) : (k + 1) * (M1 - 1), 3] = np.arange(
            k * M1 + 3, (k + 1) * M1 + 2
        )
    ...
    for k in range(f2):
        ...
        bond[f1 * (M1 - 1) + k * (M2 - 1) : f1 * (M1 - 1) + (k + 1) * (M2 - 1), 2] = (
            np.arange(f1 * M1 + 2 + k * M2, f1 * M1 + (k + 1) * M2 + 1)
        )
        bond[f1 * (M1 - 1) + k * (M2 - 1) : f1 * (M1 - 1) + (k + 1) * (M2 - 1), 3] = (
            np.arange(f1 * M1 + 3 + k * M2, f1 * M1 + (k + 1) * M2 + 2)
        )
    ...
    for k in range(f2):
        ...
        bond[f1 * (M1 - 1) + f2 * (M2 - 1) + f1 + (k + 1) - 1, 2] = (
            (f1 * M1) + k * M2 + 2
        )
    ...
    with open(file_name, "w") as f:
        ...
        f.write(
            f"LAMMPS data file for coarse grained star with (f1,M1)=({f1},{M1}) (f2,M2)=({f2},{M2})  \n\n"
        )
        f.write("%1.0f atoms\n" % num_atoms)
        f.write("%1.0f atom types\n" % atomtypes)
        f.write("%1.0f bonds\n" % num_bonds)
